...
def create_table_customer_credit_card_details():
    ...
    CREATE TBLE IF NOT EXISTS customer_credit_card_details(
    id INT,
    name VARCHAR(255),
    email VARCHAR(255),
...
def insert_data_customer_credit_card_details(json_data):
    ...
    for _, row in df.iterrows():
        insert_query = f"""
        INSERT INTO customer_credit_card_details (id,name,email,credit_cardnumber,credit_card_type)
        VALUES({row['id']},{row['name']},{row['email']},{row['credit_card_number']},{row['credit_card_type']})
        """

        pg_hook.run(insert_query)

with DAG(
        dag_id="pipeline_with_s3_hook",
        description="Executing a pipeline with an S3 Hook",
        default_args=default_args,
        schedule_interval="@once",
        start_date=days_ago(1),
        tags=["python", "hooks", "postgres_hook", "s3_hook", "s3_bucket"]
) as dag:
    ...
    read_s3_file_task = PythonOperator(
        ...
        op_kwargs={
            "bucket_name": "dq-credit-card-data",
    ...
    insert_data_customer_credit_card_details_task = PythonOperator(
        ...
        op_kwargs={
            "json_data": '{{ ti.xcom_pull(tsk_ids="remove_null_values_task")}}'
        }
    )
